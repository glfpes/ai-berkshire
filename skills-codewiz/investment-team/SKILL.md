...
description: 用 4 个并行 Agent（商业模式/财务估值/行业竞争/风险管理层）组成投研团队，同时深度研究一家公司再由 Team Lead 综合。当用户要求「最全面/多角度并行/investment-team 某公司」或希望四大师视角对抗式研究时使用。输出 reports/{公司名}/ 目录含 4 视角+最终报告。
---
...
- **财务数据必须来自两个独立来源**，按 `skills-codewiz/financial-data/SKILL.md` 规范执行（美股：macrotrends+stockanalysis；港股：aastocks+macrotrends；A股：东方财富+巨潮资讯），两源误差>1%须标记
- 确保数据准确，关键数据标注来源
- 分析要深入，不流于表面
