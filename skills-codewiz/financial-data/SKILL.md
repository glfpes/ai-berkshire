---
name: financial-data
description: 财务数据获取与交叉验证规范：每个关键数据须来自两个独立来源（美股 macrotrends+stockanalysis、港股 aastocks+macrotrends、A股 东方财富+巨潮），误差超过 1% 需标记。当需要采集/核对企业财务数据、确定数据源、做交叉验证时使用（被其他投研 skill 引用）。
---

> **输入适配（Codewiz）**：本 skill 原为 Claude Code slash command，正文中的 `$ARGUMENTS` 指用户在触发时提供的研究对象/参数（如公司名、行业、季度、持仓清单、问题）。调用本 skill 时，请把用户的具体输入代入 `$ARGUMENTS` 所在位置。
> **路径适配**：正文中 `tools/xxx.py`、`reports/`、`assets/` 等相对路径均相对于项目根目录 `ai-berkshire/`（本 skill 的上两级目录）。原文对其他 skill 的 `skills/xxx.md` 引用，在本版对应 `skills-codewiz/xxx/SKILL.md`。
