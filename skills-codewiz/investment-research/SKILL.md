...
description: 对单个上市公司做巴菲特/芒格/段永平/李录四大师综合深度投研（七模块：AI偏见自觉、数据收集、商业模式、财务估值、管理层、风险、结论）。当用户要求「深度研究/全面分析某公司值不值得投/investment-research 某公司」时使用。输出 reports/{公司名}-research-{YYYYMMDD}.md。
---
...
> **数据源规范**：参见 `skills-codewiz/financial-data/SKILL.md`。所有财务数据必须来自两个独立来源，误差>1%须标记。
...
**Step 2 — 取数核验：**
对清单中每个数据点，按 `skills-codewiz/financial-data/SKILL.md` 规范从可靠信源取数
